...
RUN mkdir "$ANDROID_HOME" .android && \
    ...
    # Install Fastlane
    apt-get update && \
    apt-get install --no-install-recommends -y --allow-unauthenticated build-essential git ruby-full imagemagick librsvg2-bin nodejs && \
    gem install rake bundler fastlane && \
    # Clean up
